...
export default function Gallery() {
    ...
    return (
        <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {IMAGES.map((image) => (
                        <div key={image.id} className="w-full h-auto">
                            <img src={image.src} alt={image.alt} className="w-full h-full object-cover" onClick={() => handleImageClick(image)} />
                        </div>
                    ))}
                </div>
            </main>
            <AnimatePresence>
                {selectedImage && (
                <motion.div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-75 z-[100] top-0 left-0 backdrop-blur-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={handleCloseModal}>
                    <motion.div className="relative max-w-[90%] md:max-w-[75%] lg:max-w-[60%] max-h-[80%] md:h-[75%] lg:max-h-[60%]" initial={{ opacity: 0, y: -100, rotateX: -60 }} animate={{ opacity: 1, y: 0, rotateX: 0 }} exit={{ opacity: 0, y: -100, rotateX: -25 }}>
                        <button className="absolute top-[-50px] right-0 md:right-[-50px] p-2 font-bold text-white text-2xl bg-red-500 rounded-full" onClick={() => setSelectedImage(null)}>
